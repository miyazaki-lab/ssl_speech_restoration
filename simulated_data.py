...
def get_arg():
    ...
    parser.add_argument("--in_dir", default="data/jsut_22k", type=pathlib.Path)
    parser.add_argument("--out_dir", default="data/jsut_22k_low", type=pathlib.Path)
    parser.add_argument(
        "--corpus_type", default="single", type=str, choices=["single", "multi"]
    )
    parser.add_argument(
        "--deg_type",
        default="aliasing",
        type=str,
        choices=["lowpass", "clipping", "mulaw", "overdrive", "aliasing"],
    )
...
def aliasing(args):
    ...
    for wp in tqdm.tqdm(wavslist):
        ...
        wav_processed = upsampler(new_wav)
        wav_out = wav_norm(wav_processed, sr)
        wav_out = wav_out.squeeze(0).numpy()
        if data_type == "single":
            sf.write(out_dir / wp.name, wav_out, sr)
